...
for n = 1:BITS;
  ...
  print(["G_Quantized_signal ", int2str(n), ".png"], "-dpng", "-color");
  
  signal_rounded = round(s * a) / a;
  
  ## audio save function from folder
  ausave(['G', int2str(n), '.wav'], signal_rounded, fp);
...
## figure = new window with plot
 ...
 file1 = ['G' , int2str(BITS), '.wav'];
 file2 = ['G' , int2str(BITS + 1), '.wav'];
...
print(['G_Fig. 1 - Plots of signals original sampled and noise.png'], "-dpng", '-S1600,900');   
...
ausave(['G_filtered.wav'], s_filtered_mono, fp); 
...
print(['G_Fig. 2 - Plots of filter FIR.png'],"-dpng", '-S1600,900'); 
...
print(['G_Fig. 3 - Plots of frequency response of filter FIR.png'],"-dpng", '-S1600,900'); 
