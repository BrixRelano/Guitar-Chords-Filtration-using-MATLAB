...
for n = 1:BITS;
  ...
  print(["C_Quantized_signal ", int2str(n), ".png"], "-dpng", "-color");
  
  signal_rounded = round(s * a) / a;
  
  ## audio save function from folder
  ausave(['C', int2str(n), '.wav'], signal_rounded, fp);
...
## figure = new window with plot
 ...
 file1 = ['C' , int2str(BITS), '.wav'];
 file2 = ['C' , int2str(BITS + 1), '.wav'];
...
print(['C_Fig. 1 - Plots of signals original sampled and noise.png'], "-dpng", '-S1600,900');   
...
ausave(['C_filtered.wav'], s_filtered_mono, fp); 
...
print(['C_Fig. 2 - Plots of filter FIR.png'],"-dpng", '-S1600,900'); 
...
print(['C_Fig. 3 - Plots of frequency response of filter FIR.png'],"-dpng", '-S1600,900'); 
