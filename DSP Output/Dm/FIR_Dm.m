## beginning - clearing all previous figures, and windows etc...
clear all;
close all;
clf;
pkg load signal;
...
## s- signal## fp - frequency of a file - for us it is 44100 Hz
[s, fp] = auload('Dm.wav');
...
## figure = new window with plot
 ...
 ylabel({'value of SNR', 'in logaritmic scale [dB]'});
 ...
 file1 = ['Dm' , int2str(BITS), '.wav'];
 file2 = ['Dm' , int2str(BITS + 1), '.wav'];
 ...
 ## you could use logarithm in this place 
    Nf_help = 1;
    help = 2;
    while(help < N)
        help = help* 2;
...
print(['Dm_Fig. 1 - Plots of signals original sampled and noise.png'], "-dpng", '-S1600,900');   
...
## frequency normalized, fp ~44.1kHz
wc = freqCut / (fp/2);

## FIR filter coefficients
firCoeff = fir1(101, wc, 'low');
...
s_filtered = filtfilt(firCoeff, 1, s_final);
...
title({"Module of signal spectrum after" , "using low-pass filer"});
xlabel('Frequency [Hz]');
...
s_filtered_mono = s_filtered(:,1);
ausave(['Dm_filtered.wav'], s_filtered_mono, fp); 




## frequency response of filter FIR
...
## frequency response phase angle of filter FIR
subplot(2,3,6);
plot(f, angle(H));
title({"Frequency response of", "phase angle"});
xlabel('Frequency [Hz]');
ylabel('Phase angle of signal [rad]');
grid on; hold on;
...
print(['Dm_Fig. 2 - Plots of filter FIR.png'],"-dpng", '-S1600,900'); 
...
print(['Dm_Fig. 3 - Plots of frequency response of filter FIR.png'],"-dpng", '-S1600,900'); 
