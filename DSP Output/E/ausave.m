...
function ausave(path, data, rate, sampleformat)
  ...
  if strcmp(ext,'wav') 
    ...
    arch = 'ieee-le';
    fwrite(file, double('RIFF'), 'char');
    fwrite(file, datasize+36, 'long', 0, arch);
    fwrite(file, double('WAVE'), 'char');

    ## write the "fmt " section
    fwrite(file, double('fmt '), 'char');
  ...
  elseif strcmp(ext,'aiff') || strcmp(ext,'aif')
    ...
    ## write the magic header
    arch = 'ieee-be';
    fwrite(file, double('FORM'), 'char');
    fwrite(file, datasize+46, 'long', 0, arch);
    fwrite(file, double('AIFF'), 'char');

    ## write the "COMM" section
    fwrite(file, double('COMM'), 'char');
